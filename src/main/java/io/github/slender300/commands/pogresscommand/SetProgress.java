...
public class SetProgress {
    ...
    public static void checkLengthSet(CommandEvent event, String[] args) {
        if (args.length == 3)
        {
            setProgress(event, args);
        }
        else
        {
            errorArgs(event);
    ...
    private static void setProgress(CommandEvent event, String[] args) {
        try
        {
            ...
            Objects.requireNonNull(event.getJDA().getVoiceChannelById(ProgressCommand
                    ...
                    .setName(String.format("%s %s %s %d%%",parts[0], parts[1], parts[2], number))
                    .queue();
            ...
            embed.setTitle(String.format("El canal de progreso ha sido colocado en %d", number));
